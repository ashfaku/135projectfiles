...
void checkPrimeCount(int a, int b) {
  cout << "There are " << countPrimes(a, b) << " primes between " << a
       << " and " << b << endl;
}
...
void checkNextTwinPrime(int x) {
  cout << "The next twin prime after " << x << " is " << nextTwinPrime(x)
       << endl;
}
void checkLargestTwinPrime(int a, int b) {
  cout << "The largest twin prime between " << a << " and " << b
       << " (including the interval bounds), is " << largestTwinPrime(a, b)
       << endl;
}
