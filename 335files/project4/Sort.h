...
void quicksort2( vector<Comparable> & a, int left, int right, Comparator less_than)
{
    if (left >= right) 
        return;
    int leftHolder = left, rightHolder = right, pivot = a[left + (right - left)/2]; // set pivot to value
    for ( ; ; )
    {
        while(less_than(a[leftHolder], pivot) )
            leftHolder++;
        while(less_than(pivot, a[rightHolder]) )
            rightHolder--;  
        if (leftHolder <= rightHolder)
        {
            swap(a[leftHolder], a[rightHolder]);
            leftHolder++;
            rightHolder--;
        }
        else    
    ...
    if(left < rightHolder)
        quicksort2(a, left, rightHolder, less_than);
    if(leftHolder < right)
        quicksort2(a, leftHolder, right, less_than);
...
void quicksort3( vector<Comparable> & a, int left, int right, Comparator less_than)
{
    if (left >= right)
        return;
    int i = left - 1, j = right + 1, pivot = a[(left + right)/2];
    for ( ; ; )
    {
        while (less_than(a[++i], pivot)) ;
        while (less_than(pivot, a[--j])) ;
    ...
    quicksort3(a, left, j, less_than);
    quicksort3(a, j + 1, right, less_than);
}
